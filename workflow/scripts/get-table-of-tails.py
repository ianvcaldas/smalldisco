...
with open(snakemake.input[0], 'r') as f:
    for line in f:
        ...
        # The following reads are uniquely mapped to a locus of interest
            ...
            if ',' in read_tags['YB']:
                these_tags = [{'YB': mapped_locus, 'TL': read_tags['TL']} for mapped_locus in read_tags['YB'].split(',')]
...
counts = pd.DataFrame.from_records(records, columns=['read_tail', 'region', 'num_reads'])
