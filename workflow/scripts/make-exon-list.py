...
annot.columns = ["chrom", "source", "feature", "start", "end", "score", "strand", "frame", "attribute"]
            
parsed_attributes = []
for ix, value in annot.attribute.iteritems():
    parsed_attributes.extend(extract(ix, value, pattern))
    
attributes_df = pd.DataFrame.from_records(parsed_attributes, columns=['index', 'attribute', 'value'])
...
attributes_df = attributes_df.pivot_table(index="index", columns="attribute", values="value", aggfunc=lambda x: ";".join(x))
...
data[bed_cols].to_csv(snakemake.output[0], sep='\t', header=False, index=False, quoting=csv.QUOTE_NONE)
